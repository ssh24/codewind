...
if [[ $MICROCLIMATE_WS_ORIGIN &&  "$APPDIR" == '/codewind-workspace'* ]]
    then
		...
		if [ $? -eq 0 ]; then
			echo -e "Listing source files on PFE container before copy"
			ls -la "$APP_DIRECTORY"

			echo -e "Copying over source files"
			docker cp "$APP_DIRECTORY"/. $CONTAINER_NAME:$HOME/app
			echo -e "Exit code of docker cp command: $?"

			echo -e "Listing source files on PFE container after copy"
			ls -la "$APP_DIRECTORY"

			echo -e "Listing source files on APP container"
			docker exec -i $CONTAINER_NAME bash -c "ls -la $HOME/app"
		fi
		...
		if [ $? -eq 0 ]; then
			echo -e "Listing source files on PFE container"
			ls -la "$APP_DIRECTORY"
